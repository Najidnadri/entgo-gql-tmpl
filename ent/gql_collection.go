...
func (pa *PaymentQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "dateCreated":
			if _, ok := fieldSeen[payment.FieldDateCreated]; !ok {
				selectedFields = append(selectedFields, payment.FieldDateCreated)
				fieldSeen[payment.FieldDateCreated] = struct{}{}
			}
		case "dateUpdated":
			if _, ok := fieldSeen[payment.FieldDateUpdated]; !ok {
				selectedFields = append(selectedFields, payment.FieldDateUpdated)
				fieldSeen[payment.FieldDateUpdated] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func newPaymentPaginateArgs(rv map[string]any) *paymentPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case []any:
			...
			args.opts = append(args.opts, WithPaymentOrder(orders))
		}
	}
